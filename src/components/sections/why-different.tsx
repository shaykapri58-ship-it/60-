    ...
    <section className="bg-secondary/60 py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        ...
        <motion.div
          ...
          className="mt-14 grid grid-cols-1 gap-5 md:mt-16 lg:grid-cols-3"
        >
          {factors.map((factor) => (
            <motion.div
              key={factor.title}
              variants={item}
              className={cn(
                factor.span === "lead" && "lg:col-span-2"
              )}
            >
...
function FactorTile({ factor }: { factor: FactorCard }) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "flex h-full min-h-[168px] flex-col justify-between rounded-[1.65rem] p-6",
          innerTone
        )}
      >
        <span
